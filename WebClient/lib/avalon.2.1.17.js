	...
	*/
	 ...
	 */
	   ...
	   var keyMap = avalon.oneObject("break,case,catch,continue,debugger,default,delete,do,else,false," +
		...
		"package,private,protected,WebClient,short,static,super,synchronized," +
	   ...
	   valiDir.defaults = {
		...
		validateInBlur: true, //@common {Boolean} true，在blur事件中进行验证,触发onSuccess, onError, onComplete回调
		validateInKeyup: true, //@common {Boolean} true，在keyup事件中进行验证,触发onSuccess, onError, onComplete回调
		validateAllInSubmit: true, //@common {Boolean} true，在submit事件中执行onValidateAll回调
		resetInFocus: true, //@common {Boolean} true，在focus事件中执行onReset回调,
		deduplicateInValidateAll: false //@common {Boolean} false，在validateAll回调中对reason数组根据元素节点进行去重
	   }
